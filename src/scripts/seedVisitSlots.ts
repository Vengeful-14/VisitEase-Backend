import { PrismaClient, SlotStatus } from '../generated/prisma';
import { CreateVisitSlotData, SlotGenerationOptions } from '../type';

const prisma = new PrismaClient();

// Helper function to create DateTime for time
const createTimeDateTime = (hours: number, minutes: number): Date => {
  const date = new Date();
...
const getRandomStatus = (): SlotStatus => {
  ...
  if (rand < 0.85) return 'booked';        // 15% booked
  if (rand < 0.95) return 'cancelled';     // 10% cancelled
...
const generateNovemberSlots = async (createdBy?: string): Promise<void> => {
  try {
    ...
    const options: SlotGenerationOptions = {
      startDate,
      endDate,
      startHour: 8,    // 8 AM
      endHour: 16,     // 4 PM
    ...
    const slotsToCreate: CreateVisitSlotData[] = [];
    ...
    while (currentDate <= endDate) {
      ...
      if (isWeekday(currentDate)) {
        ...
        for (const startTime of timeSlots) {
          ...
          const slotData: CreateVisitSlotData = {
...
const seedVisitSlots = async (): Promise<void> => {
  try {
...
export { seedVisitSlots, generateNovemberSlots };
