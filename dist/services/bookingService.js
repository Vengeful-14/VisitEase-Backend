...
class BookingService {
    ...
    async ensureUniqueTrackingToken() {
        let token;
        let isUnique = false;
        let attempts = 0;
    ...
    async getSlotForAvailability(slotId) {
        const slot = await this.prisma.visitSlot.findUnique({
            where: { id: slotId },
            select: {
                id: true,
        ...
        if (!visitor) {
            visitor = await this.prisma.visitor.create({
                data: {
                    ...
                    visitorType: bookingData.visitor.visitorType || 'individual',
    ...
    async trackBooking(email, trackingToken) {
    ...
    async cancelPublicBooking(email, trackingToken, reason) {
